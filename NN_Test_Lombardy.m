...
for i = 1:NumOutputs
    corr_pers3(i) = 1 - sum((output_pers(i,:) - PM10_target_pers_val(i,:)).^2)/(sum((mean(PM10_target_pers_val(i,:)) - PM10_target_pers_val(i,:)).^2));
    nrmse_pers3(i) = sqrt((sum((PM10_target_pers_val(i,:)-output_pers(i,:)).^2))/(sum((output_pers(i,:)-mean(output_pers(i,:))).^2))); 
end

err_medio_abs_norm_pers3 = 1/width(output_pers)*(sum(abs(PM10_target_pers_val-output_pers),2)./mean(abs(PM10_target_pers_val),2));
...
for i = 1:NumOutputs
    corr_val3(i) = 1 - sum((output_val(i,:) - PM10_target_val(i,:)).^2)/(sum((mean(PM10_target_val(i,:)) - PM10_target_val(i,:)).^2));
    nrmse_val3(i) = sqrt((sum((PM10_target_val(i,:)-output_val(i,:)).^2))/(sum((output_val(i,:)-mean(output_val(i,:))).^2))); 
   
    corr_test3(i) = 1 - sum((output_test(i,:) - PM10_target_test(i,:)).^2)/(sum((mean(PM10_target_test(i,:)) - PM10_target_test(i,:)).^2));
    nrmse_test3(i) = sqrt((sum((PM10_target_test(i,:)-output_test(i,:)).^2))/(sum((output_test(i,:)-mean(output_test(i,:))).^2))); 
...
err_medio_abs_norm_test3 = 1/width(output_test)*(sum(abs(PM10_target_test-output_test),2)./mean(abs(PM10_target_test),2));
err_medio_abs_norm_val = 1/width(output_val)*(sum(abs(PM10_target_val-output_val),2)./mean(abs(PM10_target_val),2));
...
tiledlayout(2,2);
nexttile;
plot(1:NumOutputs, [corr_test; corr_test2; corr_test3; corr_pers], '.-', 'MarkerSize', 20, 'LineWidth', 2);
grid on;
xticks(1:NumOutputs);
ylim([-0.5, 1]);
xlabel('Days'); ylabel('R^2');
...
nexttile;
plot(1:NumOutputs, [nrmse_test; nrmse_test2; nrmse_test3; nrmse_pers], '.-', 'MarkerSize', 20, 'LineWidth', 2);
grid on;
xticks(1:NumOutputs);
xlabel('Days'); ylabel('NRMSE');
ylim([0, 1.5]);
...
nexttile;
plot(1:NumOutputs, [err_medio_abs_norm_test1, err_medio_abs_norm_test2, err_medio_abs_norm_test3, err_medio_abs_norm_pers], '.-', 'MarkerSize', 20, 'LineWidth', 2);
grid on;
xticks(1:NumOutputs);
ylim([0, 0.6]);
xlabel('Days'); ylabel('NMAE');
...
p_ax=ax.Position;
area([NaN NaN], NaN(2, 4));
leg = legend('CNN', 'FFN', 'LSTM', 'Persistence model');
p_leg=leg.Position;
delete(ax)
ax=axes('Position',[p_ax(1:2) 0 0]);
area([NaN NaN], NaN(2, 4));
...
% path = strcat(outFOLD, 'Stat_comparison_', dm, nt, '.png');
% saveas(gcf, path);
